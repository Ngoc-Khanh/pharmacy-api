...
class ReviewController extends Controller
{
     ...
     *     description="Trả về danh sách tất cả đánh giá sản phẩm",
     *     @OA\Response(
     *         response=200,
    ...
    {
        $reviews = Review::all();
        return $this->json($reviews, 'Danh sách đánh giá');
    }
     ...
     *         @OA\JsonContent(
     *             @OA\Property(property="success", type="boolean", example=true),
     *             @OA\Property(property="message", type="string", example="Đánh giá đã được xóa thành công"),
     *             @OA\Property(property="data", type="null", example=null)
     *         )
     *     ),
     *     @OA\Response(
     ...
     *         @OA\JsonContent(
     *             @OA\Property(property="success", type="boolean", example=false),
     *             @OA\Property(property="message", type="string", example="Bạn không có quyền xóa đánh giá này"),
     *             @OA\Property(property="data", type="null", example=null)
     *         )
     *     ),
     *     @OA\Response(
    ...
    public function deleteReview($id)
    {
        ...
        if (!$review) return $this->fail(null, 'Đánh giá không tồn tại', 404);
        $user = Auth::user();
        if ($review->user_id !== $user->_id) return $this->fail(null, 'Bạn không có quyền xóa đánh giá này', 403);
        $review->delete();
        return $this->json(null, 'Đánh giá đã được xóa thành công');
    }
}
